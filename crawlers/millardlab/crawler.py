...
class MillardLabPhagesCrawler:
    ...
    def __init__(self, url: str):
        ...
        self.columns = [
            'Accession', 'Description', 'Classification',
            'Genome Length(bp)', 'molGC'
        ]
    ...
    def _process_html(self) -> List:
        ...
        try:
            ...
            acc_numbers = self._process_element(
                document,
                "//td[contains(@class, 'column-1')]/a"
            )

            # Description
            descriptions = self._process_element(
                document,
                "//td[contains(@class, 'column-2')]"
            )

            # Classification
            taxonomies = self._process_element(
                document,
                "//td[contains(@class, 'column-3')]"
            )

            # Genome Length(bp)
            bps = self._process_element(
                document,
                "//td[contains(@class, 'column-4')]"
            )

            # molGC
            gcs = self._process_element(
                document,
                "//td[contains(@class, 'column-5')]"
            )
